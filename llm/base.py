...
class LLM:
    ...
    def __init__(self, args):
        ...
        self.model, self.tokenizer = load_model(args.model, 
            ...
            flash_attention_2=(args.attn_implementation == 'flash_attention_2')
        )
    ...
    def generate(self, prompt, max_tokens, stop=None):
        ...
        # generation = self.citation_fixing(generation)
        return generation

    def postprocess(self, x_):
        x = x_.split('\n\n\n')[0] 
        x = x.split('Reference:')[0] 
        x = x.split('References:')[0] 
        x = x.split('Note:')[0] 
        x = x.split('Notes:')[0] 
        x = x.split('Citation:')[0] 
        x = x.split('Citations:')[0] 
        x = x.split('Source:')[0] 
        x = x.split('Sources:')[0] 
        return x
